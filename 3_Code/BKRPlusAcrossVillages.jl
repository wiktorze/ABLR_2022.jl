...
baseline = DataFrame(StatFiles.load("1_Data/StataFiles/TFP&TFPRpacrossvil.dta"))
# Save as csv
# CSV.write("1_Data/StataFiles/TFP&TFPRpacrossvil.csv", baseline)
dropmissing!(baseline, [:hhid])

# Merge the two dataframes
df_merged = outerjoin(df, baseline, on=:hhid)
df_merged[df_merged.hhid .== 1402004, :TFPRfe]
df_merged.TFPRavg = log.(df_merged.TFPRfe)
...
# Regression
ols = reg(df_merged, @formula(DlogY ~ 0 + logTFPRmeasure + DlogI + interact + year + village), save = true, Vcov.cluster(:village))
...
t1_col2 = [string(lambda); "($std_error)"; "[$lci, $hci]"]

# save
@save "2_Intermediate/BKRPlusAcrossVillages/t1_col2_BKR.jld2" t1_col2